...
class SPP(nn.Module):
    def __init__(self, places=None, size=None):
        ...
        if size is not None:
            ...
            self.conv = Conv(in_channels=places, out_channels=places, kernel_size=3, stride=1)
...
class YOLOLayer(nn.Module):
    ...
    def compute_grid_offsets(self, grid_size, cuda=True):
        ...
        self.grid_x = torch.arange(gw).repeat(gh, 1).view([1, 1, gh, gw]).type(FloatTensor)
        self.grid_y = torch.arange(gh).repeat(gw, 1).t().view([1, 1, gh, gw]).type(FloatTensor)
...
class loss_layer(nn.Module):
    ...
    def forward(self, xs, target):
        ...
        for i, xs_ in enumerate(xs):
            ...
            if target.size(0):
                obj_mask = FloatTensor(nB, nA, nGh, nGw).fill_(0)
                noobj_mask = FloatTensor(nB, nA, nGh, nGw).fill_(1)

                tx = FloatTensor(nB, nA, nGh, nGw).fill_(0)
                ty = FloatTensor(nB, nA, nGh, nGw).fill_(0)
                tw = FloatTensor(nB, nA, nGh, nGw).fill_(0)
                th = FloatTensor(nB, nA, nGh, nGw).fill_(0)
                ...
                for i, iou in enumerate(anchors_iou.t()):
                    noobj_mask[b[i], iou > self.ignore_thres, gj[i], gi[i]] = 0

                tx[b, best_n, gj, gi] = gx - gx.floor()
                ty[b, best_n, gj, gi] = gy - gy.floor()
                tw[b, best_n, gj, gi] = torch.log(gw / anchor[best_n][:, 0] + 1e-16)
                th[b, best_n, gj, gi] = torch.log(gh / anchor[best_n][:, 1] + 1e-16)
                ...
                tconf = FloatTensor(nB, nA, nGh, nGw).fill_(0)
                conf_bce = self.cross_entropy(pred_conf, tconf)
...
class ResNet(nn.Module):
    def __init__(self, anchors, blocks=[3, 4, 5, 5], expansion=2, Istrain=True):
        ...
        print('\n\nUse spp model.\n')
        self.spp1 = SPP(places=256, size=2)
        self.spp2 = SPP(places=256, size=4)
        self.spp3 = SPP()
        self.spp = Conv(in_channels=256*4, out_channels=256, kernel_size=3, stride=1)
    ...
    def forward(self, x):
        ...
        spp2 = self.spp2(x4)
        spp3 = self.spp3(x4)
        spp = torch.cat([x4, spp1, spp2, spp3], 1)
        x = self.spp(spp)
